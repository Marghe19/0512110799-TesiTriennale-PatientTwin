...
def main():
    ...
    heart = np.vstack([heartA, heartB, heartC, heartD])
    ...
    sns.kdeplot(heartsr[:498, 0], heartsr[:498, 1], cmap="autumn_r", n_levels=20, shade=True, shade_lowest=False, alpha=0.8)
    sns.kdeplot(heartsr[498:996, 0], heartsr[498:996, 1], cmap="Greens_r", n_levels=20, shade=True, shade_lowest=False, alpha=0.4)
    # sns.kdeplot(heartsr[996:1494, 0], heartsr[996:1494, 1], cmap="Oranges_r", n_levels=20, shade=True, shade_lowest=False, alpha=0.4)
    # sns.kdeplot(heartsr[1494:, 0], heartsr[1494:, 1], cmap="Blues_r", n_levels=20, shade=True, shade_lowest=False, alpha=0.4)
    ...
    a2 = FancyArrowPatch((-1.5, 0.8), (-3, -0.2), connectionstyle="arc3,rad=0.4", color='black', **kw)
    ax.add_patch(a2)
    plt.text(-2.9, 1.1, 'treatment', ha='left', rotation=0, wrap=True)
    plt.xlim([-3.5, 5])
    plt.ylim([-3.1, 3.5])
    plt.xlabel('1st principal component')
    plt.ylabel('2nd principal component')
    plt.tight_layout()
    plt.savefig(f"./results/heart_therapy.png")
    plt.savefig(f"./results/heart_therapy.pdf")
    plt.show()

    fig, ax = plt.subplots(figsize=[5, 3])
    # sns.kdeplot(heartsr[:498, 0], heartsr[:498, 1], cmap="autumn_r", n_levels=20, shade=True, shade_lowest=False, alpha=0.8)
    sns.kdeplot(heartsr[498:996, 0], heartsr[498:996, 1], cmap="Greens_r", n_levels=20, shade=True, shade_lowest=False, alpha=0.4)
    sns.kdeplot(heartsr[996:1494, 0], heartsr[996:1494, 1], cmap="Reds_r", n_levels=20, shade=True, shade_lowest=False, alpha=0.4)
    sns.kdeplot(heartsr[1494:, 0], heartsr[1494:, 1], cmap="Oranges_r", n_levels=20, shade=True, shade_lowest=False, alpha=0.4)
    sns.despine(left=True, bottom=True)
    style = "Simple,tail_width=0.5,head_width=4,head_length=8"
    kw = dict(arrowstyle=style)
    a1 = FancyArrowPatch((1.5, -2.5), (3.5, -0.8), connectionstyle="arc3,rad=0.4", color='black', **kw)
    ax.add_patch(a1)
    plt.text(3, -2.5, 'viral infection', ha='left', rotation=0, wrap=True)
    a2 = FancyArrowPatch((-0.5, 2.1), (-1.9, 0.8), connectionstyle="arc3,rad=0.4", color='black', **kw)
    ax.add_patch(a2)
    ...
    plt.ylabel('2nd principal component')
    plt.tight_layout()
    plt.savefig(f"./results/heart_infection.png")
    plt.savefig(f"./results/heart_infection.pdf")
    plt.show()
